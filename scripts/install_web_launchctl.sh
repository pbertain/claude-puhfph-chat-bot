#!/bin/bash
# Install launchctl service for app status interface
...
echo "Installing app status launchctl service..."

# Create LaunchAgents directory if it doesn't exist
mkdir -p "$HOME/Library/LaunchAgents"

# Create log files if they don't exist (launchctl requires them to exist)
touch "$REPO_DIR/app_status.log"
touch "$REPO_DIR/app_status_error.log"
...
echo "App status service installed and started!"
echo ""
echo "To check status: launchctl list | grep claudepuhfph"
echo "To stop: launchctl unload $PLIST_DEST"
echo "To start: launchctl load $PLIST_DEST"
echo "To view logs: tail -f $REPO_DIR/app_status.log"
echo ""
echo "Access the app status interface at: http://localhost:55042"
